...
lb.imshow_test(model, test_loader, show_reference=True, show_original=True)
...
for t in range(epochs):
    ...
    loss, train_accuracy = train(train_loader, model, loss_fn, opt, 
                                 scheduler=scheduler, scaler=torch.cuda.amp.GradScaler())
    test_loss, test_accuracy = test(test_loader, model, loss_fn)
    lb.imshow_test(model, test_loader, show_reference=False)
